...
class Enemy(Player):
    ...
    def shoot(self):
        bullet = Bullet(self.rect.centerx, self.rect.bottom, 's')
        all_sprites.add(bullet)
        bullets.add(bullet)
...
def check_collide():
    ...
    if pygame.sprite.groupcollide(enemies, bullets, False, True):
        enemy.hp -= 5
        if enemy.hp == 0:
            enemy.kill()

    if pygame.sprite.groupcollide(landscape, bullets, False, True):
        wall.hp -= 5
